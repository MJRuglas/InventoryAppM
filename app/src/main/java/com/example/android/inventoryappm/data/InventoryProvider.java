...
public class InventoryProvider extends ContentProvider {
    ...
    private Uri insertBook(Uri uri, ContentValues values) {
        ...
        Integer price = values.getAsInteger(inventory.COLUMN_PRODUCT_PRICE);
        if (price != null && price < 0) {
            throw new IllegalArgumentException("Book requires valid quantity");
        }

        // If the quantity is provided, check that it's greater than or equal to 0 kg
        Integer quantity = values.getAsInteger(inventory.COLUMN_PRODUCT_QUANTITY);
        if (quantity != null && quantity < 0) {
            throw new IllegalArgumentException("Book requires valid quantity");
        }

        String supplier = values.getAsString(inventory.COLUMN_SUPPLIER_NAME);
        if (supplier == null) {
            throw new IllegalArgumentException("Book requires a supplier");
        }

        // If the quantity is provided, check that it's greater than or equal to 0 kg
        Integer phone = values.getAsInteger(inventory.COLUMN_PHONE_NUMBER);
    ...
    private int updateBook(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(inventory.COLUMN_PRODUCT_NAME)) {
            String name = values.getAsString(inventory.COLUMN_PRODUCT_NAME);
            if (name == null) {
                throw new IllegalArgumentException("Book requires a name");
            }
        ...
        // If the {@link inventory#COLUMN_PRODUCT_PRICE} key is present,
        // check that the weight value is valid.
        if (values.containsKey(inventory.COLUMN_PRODUCT_PRICE)) {
        ...
        // If the {@link inventory#COLUMN_PRODUCT_QUANTITY)} key is present,
        // check that the weight value is valid.
        if (values.containsKey(inventory.COLUMN_PRODUCT_QUANTITY)) {
            // Check that the weight is greater than or equal to 0 kg
            Integer quantity = values.getAsInteger(inventory.COLUMN_PRODUCT_QUANTITY);
            if (quantity != null && quantity < 0) {
        ...
        // If the {@link inventory#COLUMN_PRODUCT_QUANTITY)} key is present,
        // check that the weight value is valid.
        if (values.containsKey(inventory.COLUMN_PHONE_NUMBER)) {
            // Check that the weight is greater than or equal to 0 kg
            Integer phone = values.getAsInteger(inventory.COLUMN_PHONE_NUMBER);
            if (phone != null && phone < 0) {
                throw new IllegalArgumentException("Book requires valid amount");
